...
def play_song(song_name: StringVar, songs_list: Listbox, status: StringVar):
    ...
    status.set(f"Song PLAYING: {os.path.basename(selected_song)}")
...
def load(listbox):
    ...
    tracks = [track for track in os.listdir(directory) if track.endswith(".mp3")]

    listbox.delete(0, END)  # Clear the listbox

    for track in tracks:
        listbox.insert(END, os.path.join(directory, track))
...
for track in tracks:
    playlist.insert(END, os.path.join(os.getcwd(), track))
